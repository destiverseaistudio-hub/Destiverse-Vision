...
import { getWatchProgress, setWatchlistState, saveWatchProgress } from "@/services/library"
...
export default function ContentDetailPage() {
  ...
  useEffect(() => {
    ...
    void getWatchProgress()
      .then((entries) => {
        if (!active) return

        const entry = entries.find(
          (candidate) => candidate.content_id === content.id,
        )
        ...
        if (
          ...
          setResumePosition(entry.position_seconds)
        } else {
          setResumePosition(0)
        }
      })
  ...
  const relatedContent = getRelatedContent(content, allContent)
  ...
  return (
    <main className="w-full min-w-0 overflow-x-hidden pb-16">
      <section className="relative overflow-hidden border-b border-white/[0.06]">
        ...
        <div className="relative mx-auto w-full min-w-0 max-w-[var(--dv-content-max-width)] px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
          <Link
            to="/dashboard/categories"
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-end xl:grid-cols-[minmax(0,1fr)_340px]">
            <div className="min-w-0 max-w-3xl">
              ...
              <h1 className="break-words text-3xl font-black leading-[1.05] tracking-[-0.03em] text-white sm:text-4xl md:text-5xl lg:text-6xl">
                {content.title}
              </h1>
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.06] px-5 py-3 text-sm font-semibold text-white backdrop-blur-md transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <div className="mx-auto w-full max-w-[280px] lg:max-w-none">
              <div
                className={`relative aspect-[2/3] overflow-hidden rounded-3xl border border-white/10 shadow-2xl ${content.artworkClass} ${content.posterUrl ? "bg-cover bg-center" : ""}`}
      ...
      {showPlayer ? (
        <section className="mx-auto max-w-[var(--dv-content-max-width)] px-4 pt-8 sm:px-6 lg:px-8 lg:pt-10">
          <div className="mb-4 flex items-center justify-between gap-4">
            ...
            <button
              ...
              className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-xs font-medium text-white/60 transition hover:border-white/20 hover:text-white"
            >
              Close player
            </button>
          </div>
          ...
          <VideoPlayer
            ...
            onPause={(positionSeconds, durationSeconds) => {
            ...
            onEnded={(positionSeconds, durationSeconds) => {
              void saveWatchProgress(
                content.id,
                positionSeconds,
                durationSeconds,
