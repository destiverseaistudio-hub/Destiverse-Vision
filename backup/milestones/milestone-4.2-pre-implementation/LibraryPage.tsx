...
export default function LibraryPage() {
  ...
  useEffect(() => {
    void getWatchProgress().then(setProgress).catch(() => setProgress([]))
  }, [])
    ...
    <main className="space-y-8 pb-4">
      ...
      {continueWatching.length > 0 ? (
        <section>
          ...
          <div className="flex gap-3 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:gap-4">{continueWatching.map(({ item, progress: entry }) => <div className="min-w-[220px]" key={item!.id}><ContentCard item={item!} /><div className="mt-2 h-1 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-[var(--dv-accent)]" style={{ width: `${Math.min(100, (entry.position_seconds / entry.duration_seconds) * 100)}%` }} /></div></div>)}</div>
        </section>
      ) : null}
