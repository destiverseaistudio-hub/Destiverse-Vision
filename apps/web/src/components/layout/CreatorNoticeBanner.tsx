...
export default function CreatorNoticeBanner() {
  ...
  useEffect(() => { if (!session) return; const load = async () => { const { data } = await supabase.from("user_notifications").select("id,title,message,action_url").is("read_at", null).order("created_at", { ascending: false }).limit(1).maybeSingle(); setNotice(data) }; void load(); const channel = supabase.channel(`creator-notices-${crypto.randomUUID()}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "user_notifications", filter: `user_id=eq.${session.user.id}` }, load).subscribe(); return () => { void supabase.removeChannel(channel) } }, [session])
